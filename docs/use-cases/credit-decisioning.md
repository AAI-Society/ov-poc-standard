...
a live gate verifies that the exact approved model, in its approved
configuration, is what will serve. If that verification fails, no decision is issued,
...
show *after the fact* which model ran on a given application. But that is
...
- Tier 4 shows the *approved* model served. It does **not** show the approved
