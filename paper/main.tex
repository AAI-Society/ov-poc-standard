...
In the meantime we ran an automated independent pass over all 1{,}000 coded
rows, and it is worth reporting what came back because it was not flattering.
It proposed 192 changes and found 96 citations that do not name a checkable
clause. Three findings we verified ourselves. Most citations point at a chapter
heading rather than a requirement---for two frameworks, essentially all of
them---so a reader cannot look up what we claim. Six hundred and ninety-two of
the thousand rows carry a rationale repeated word-for-word on four or more
rows, which is the mechanical signature of coding by block rather than by
requirement. And every one of the 61 Exact matches was challenged, which in
hindsight we should have caught ourselves: if our thesis is that no existing
framework requires operator-independent evidence, then a sheet claiming 61
exact matches was arguing against this paper. We have removed the ones we could
verify were wrong on their face---the C10 conformance-statement rows, which
describe an artifact this standard defines and no other framework can therefore
match exactly.

We did \emph{not} apply the 91 proposed downgrades that turn on what a
framework's clause actually says. Those move the coverage numbers, and they
move them downward, which widens the gap this paper exists to describe. That is
exactly the direction a motivated coder drifts, so they wait for a human with
the source document open. The full review is published with the
repository.
