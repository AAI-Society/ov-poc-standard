...
% The Society's mark is NOT drawn here. The cover uses the official brand-kit
% lockup, figures/aais-lockup.png. Brand kit: nothing may be redrawn by hand.
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  %% neon band removed: canon rations neon to the mark and the highlight
  \node[anchor=north west, inner sep=0pt]
    at ([xshift=1in, yshift=-1.05cm]current page.north west)
    {\includegraphics[width=8.4cm]{figures/aais-lockup.png}};
  \node[anchor=north west, inner sep=0pt, text width=16.5cm]
    at ([xshift=1in, yshift=-2.30cm]current page.north west)
    {\normalfont\sffamily\footnotesize\color{aaimuted}
     Proof-of-Control Initiative $\cdot$ Working draft v0.1 $\cdot$ August 2026
     $\cdot$ for co-author review};
  \node[anchor=north west, inner sep=0pt, text width=\textwidth]
    at ([xshift=1in, yshift=-3.05cm]current page.north west)
  ...
  \node[anchor=north west, inner sep=0pt, text width=\textwidth]
    ...
    {\sffamily\small\color{aaipaper}%
     ...
     Verify, rather than promise, that your agents have the right controls in place.};
\end{tikzpicture}
...
{\small\noindent
\textbf{Tricia Wang}\textsuperscript{1} \quad
\textbf{Ken Huang}\textsuperscript{1,2} \quad
\textbf{Jim Schwoebel}\textsuperscript{1,3}

\smallskip\noindent
{\footnotesize
\textsuperscript{1}\,Advanced AI Society,
\url{https://advancedaisociety.org}\footnote{Prepared within the
...
added only upon explicit consent and substantive contribution.} \quad
\textsuperscript{2}\,Cloud Security Alliance \quad
\textsuperscript{3}\,Quome}}

\smallskip\noindent
{\footnotesize Specification, reference implementation, and all artifacts:
\url{https://github.com/LFDT-ProofOfControl/ov-poc-standard}}

\vspace{0.9em}
\noindent\textcolor{aaineutral}{\rule{\textwidth}{0.8pt}}
\vspace{0.2em}
...
ten steps or five thousand. Eleven attacks work against the naive design and fail
...
saying for fifty years. Because an auditor should be able to verify one action
...
We call this the \emph{Verifiability Gap}: the absence of open evidence
...
how thoroughly a claim was verified.
...
Section~\ref{sec:merkle}---logarithmic proofs that let an auditor verify one
...
requirement C7.3.5. It is worth noting how it was found: not by analysis,
but by implementing a performance optimization carefully enough to
...
\section{Why the Obvious Answers Do Not Work}
...
comes back, and changes its mind. So you cannot verify compliance before it
runs; there is nothing fixed to verify.

Worse, you cannot verify it one step at a time either, and this is the part
...
\paragraph{Guardrails inside the application.} Put the checks in the agent's % <!--aais-allow-->
...
\emph{the thing being verified is doing the verifying}.

\subsection{Why looking afterward is too late}

The usual backstop is the audit: collect the logs, look later. That has gone
wrong in two ways.
...
operation must go through a mechanism that is ``tamper-proof'', always invoked, % <!--aais-allow-->
...
you must say how independently that evidence can be verified; and you must say
...
verify---are grouped into six domains:
...
verified and what residual trust must be disclosed.
...
written. At Tier~3 the evidence is produced by the mechanism itself and
anyone can verify it with published tools; the parties you still depend on are
disclosed rather than removed. At Tier~4, checking is not optional---the
...
evidence a stranger can verify. Requirement levels 1--4 line up one-for-one
with the tiers.}
\label{fig:ladder}
\end{figure}

Separately from the evidence, we grade how carefully the \emph{claim} was
verified: the operator says so (Self-Declared), an accredited assessor verified
(Third-Party Assessed), or it is verified continuously as the system runs
...
human). Figure~\ref{fig:flow} shows the same boundary from the evidence
side.
...
the agent and the hooks is the \emph{Attester}; the component holding the runtime,
...
\emph{history binds}: the step number and the running total tie this
...
``which rules were in force?'' is something you verify, not something you
reconstruct from a change log and hope.

\paragraph{How you verify one.} It is mechanical, and worth walking through
...
Attestation Token claims~\cite{rfc9711}; ``PoC'' rows are the \poc{} % <!--aais-allow-->
...
While this section was in draft, the experiment it argues for was run by a
stranger. An independent engineer implemented a verifier for the KYA-OS
delegation credential from the specification text alone, with no shared code
and no say by the specification's authors in who the implementer would be,
and verified it against credentials issued by the reference implementation:
valid credentials verify, tampered ones fail, and the JCS (RFC 8785)
canonicalization and Ed25519 signature handling agree byte-for-byte across
the two implementations~\cite{izurutoys2026}. The same exercise surfaced
conformance gaps in the reference implementation, two of which were fixed and
shipped within days; that is the review loop working, not the absence of
defects. It is one implementation and one credential format, a data point
rather than a study; we record it because independent reconstruction from a
specification is the strongest test a canonical form gets, and it is the only
instance of its kind we have. Our own vectors have not yet had it.
...
let a system like this drift into implying more than it shows, and we would
...
you can verify without having watched. The evidence is the contribution; the
...
The cost per step is $O(|\tilde{\Pi}| + B)$, flat no matter
...
Constraints are inherited; sub-agent evidence verified by several
...
to ask whether that is realistic.
...
somebody outside the company can verify, or not in as much detail.
...
the NIST risk framework---which scores 59\%, second highest overall---is one of
...
how independently it can be verified, or requires you to write down what you
...
\toprule
\textbf{Property} & \textbf{Prompts / guardrails} & \textbf{Static RBAC} & % <!--aais-allow-->
...
A stranger can verify it & No & No & With operator access & Yes &
Yes (Tier 3+) \\
...
how you find out whether the arguments above survive contact with a
computer. The code is open source in the specification
repository~\cite{pocspec}, at \url{https://github.com/LFDT-ProofOfControl/ov-poc-standard}.
...
told it ``don't send after reading something sensitive,'' and it enforced % <!--aais-allow-->
...
monitor can verify: only a trusted redaction tool clears the label, and it
...
declare a migration path---so a verifier knows what to verify and an operator
...
the central claim untested: that a verifier can establish \emph{which code
...
authentic; the quoting enclave, that it signs reports honestly; Google, that the % <!--aais-allow-->
host does not extract domain memory by some route outside the TDX threat model,
and that the firmware measured into the runtime registers is what it says; % <!--aais-allow-->
...
\SI{7}{\micro\second} to verify. Replaying the chain to learn the same fact
...
\caption{What an auditor must download to verify a single action. One line is
...
\SI{44.8}{\giga\byte}---and costs the same whether you wanted to verify one
...
the length felt like checking the anchor. The two are not the same.
Comparing roots catches the rewrite immediately, and a consistency proof does better still: it settles
...
disclosed statement leaks. ``We use ZKPs'' is not a claim anyone can verify---%
...
can be \emph{publicly verifiable}---anyone may verify it with published
...
you must still trust are disclosed rather than removed; the claim of
trustlessness has been struck from the normative text, where it occurred once.
The word survives only in ``trustless setup'', the standard name for a setup
ceremony nobody has to trust.
...
(\url{https://github.com/LFDT-ProofOfControl/ov-poc-standard}) containing the
...
agent; it is a conversation with the party you would be verifying.
...
\emph{IETF RATS~\cite{ietfrats}:} an Internet-Draft establishing an AI-agent EAT
...
\emph{ACS:} \poc{} supplies the missing cryptographic evidence for
