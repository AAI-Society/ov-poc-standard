...
domains of verification covering 118 testable requirements; four tiers that
...
information flow have been saying for fifty years. Finally, because evidence
has to be believable long after it is written, we measure what it costs to
make it survive quantum computers: the hash chain already does, and swapping
the signatures for FIPS~204 ML-DSA costs a factor of 38 in signature
size---a storage bill, not a cryptography problem.}
...
verification with 118 testable requirements, four evidence properties, four
...
attack harness that runs eight attacks with and without each defense,
measurements of what it all costs, and what it takes to make the evidence
outlive the signature scheme that protects it.
\end{enumerate}
...
The requirements---118 of them, each phrased as something you can go and
...
it. So we took all 118 requirements and, one at a time, asked of eight
...
\emph{None}: they do not address it. Coverage is (Exact + Partial) over 118.
...
\textbf{Coverage} \\
\midrule
NIST AI RMF~\cite{nistairmf} & 0 & 73 & 45 & 62\% \\
OWASP AISVS~\cite{owaspaisvs} & 16 & 57 & 45 & 62\% \\
ISO/IEC 42001~\cite{iso42001} & 8 & 64 & 46 & 61\% \\
SOC 2~\cite{soc2tsc} & 8 & 59 & 51 & 57\% \\
EU AI Act~\cite{euaiact} & 8 & 57 & 53 & 55\% \\
CSA AARM~\cite{csaaarm} & 13 & 46 & 59 & 50\% \\
Zero Trust (SP 800-207)~\cite{sp800207} & 8 & 44 & 66 & 44\% \\
MITRE ATLAS~\cite{mitreatlas} & 0 & 31 & 87 & 26\% \\
\bottomrule
\end{tabular}
...
\midrule
Ed25519 (classical) & 64 B & 32 B & 928 B & \SI{0.93}{\giga\byte} \\
ML-DSA-44 & 2{,}420 B & 1{,}312 B & 10{,}352 B & \SI{10.35}{\giga\byte} \\
ML-DSA-65 & 3{,}309 B & 1{,}952 B & 13{,}908 B & \SI{13.91}{\giga\byte} \\
Hybrid Ed25519 + ML-DSA-44 & 2{,}484 B & 1{,}344 B & 10{,}608 B & \SI{10.61}{\giga\byte} \\
\bottomrule
\end{tabular}
...
whole record grows about eleven-fold, from roughly \SI{0.9}{\giga\byte} to
\SI{10}{\giga\byte} per million actions. That is the real bill, and it is a
...
\SI{191}{\micro\second} and is not the problem, that the policy design costs
42\% of your legitimate work and is, and that making the whole thing survive
quantum computers is a storage bill rather than a redesign. The specification, the code, the
