...
definitions say that at Tier 3 there is nobody left to trust, and this list is
the plain refutation of that sentence---which we take up in
...
\paragraph{Our tier definitions overclaim, and we now have the receipts.} The
standard says that at Tier 3 there is nobody left to trust. \S\ref{sec:tdx}
...
the first while leaving a vendor chain intact underneath the second. The tiers
should be defined by which assumptions remain at each level and stated
explicitly in every conformance claim, and until they are, a Tier 3 claim says
less than its own definition promises. We would rather print that here than let
it be discovered.
